// VNTyping.cpp : Implementation of DLL Exports.


// Note: Proxy/Stub Information
//      To build a separate proxy/stub DLL, 
//      run nmake -f VNTypingps.mk in the project directory.
...
BOOL WINAPI DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID /*lpReserved*/)
{
    ...
    {
        _Module.Init(ObjectMap, hInstance, &LIBID_VNTYPINGLib);
...
{
	SharedMem* pSharedMem = GetSharedMem();

	// Init shared memory
	pSharedMem->hMainDlg = NULL;
	pSharedMem->hEnIcon = NULL;
	pSharedMem->hVietIcon = NULL;
	pSharedMem->iconShown = 0;
	pSharedMem->keyHook = hKeyHook;
	pSharedMem->mouseHook = hMouseHook;
	pSharedMem->keyMode = CharsetTable[1].id;
	pSharedMem->iconMsgId = 0;
	pSharedMem->switchKey = 0;
	pSharedMem->vietKey = 1;
	pSharedMem->inMethod = 0;

	pSharedMem->options.freeMarking = 0;
	pSharedMem->options.toneNextToVowel = 0;
	pSharedMem->options.modernStyle = 0;
	pSharedMem->options.macroEnabled = 0;
	pSharedMem->options.useUnicodeClipboard = 0;
	pSharedMem->options.alwaysMacro = 0;
	pSharedMem->options.useIME = 0;

	pSharedMem->codeTable.encoding = CharsetTable[1].encoding;
	pSharedMem->codeTable.singleBackspace = 0;

	BuildCodeTable(CharsetTable[1].id, 0, &pSharedMem->codeTable);
